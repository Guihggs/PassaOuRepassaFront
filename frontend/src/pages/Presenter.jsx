          ...
          {[
            ...
            <button
              ...
              className={`px-4 py-2 rounded-lg text-sm font-semibold transition ${
                tab === key ? "bg-volt text-white" : "bg-panel text-white/70 hover:text-white"
              }`}
            >
              {label}
            </button>
  ...
  return (
    <div className="grid md:grid-cols-3 gap-6">
      <div className="md:col-span-2 space-y-4">
        ...
        {state.status === "live" && state.pressOrder.length > 0 && (
          <div className="rounded-2xl bg-panel border border-cyan/50 p-5 flex items-center justify-between animate-pulse">
            <div>
              <p className="text-white/50 text-xs uppercase tracking-widest">Aguardando as outras equipes…</p>
              <p className="font-display text-lg">
                {state.pressOrder.length} de 3 equipe(s) já apertaram — ordem trava em até 5s
              </p>
            </div>
          </div>
        )}
    ...
    <div className="space-y-4">
      <div className="flex justify-between items-center">
        ...
        <button
          ...
          className="px-4 py-2 rounded-lg bg-volt text-white font-semibold text-sm"
        >
          + Nova pergunta
        </button>
      </div>
  ...
  return (
    <div className="rounded-2xl bg-panel border border-volt/50 p-5 space-y-3">
      ...
      <div className="flex gap-3 justify-end">
        ...
        <button onClick={() => onSave(form)} className="px-4 py-2 rounded-lg bg-volt text-white font-semibold text-sm">Salvar</button>
      </div>
    </div>
  );
}
...
const TEAM_COLOR_OPTIONS = [
  "#0D3F94", "#DC1E2A", "#10B981", "#F4C10F", "#8B5CF6", "#FF4D6D",
];
    ...
    <div className="w-full space-y-3 text-left">
      ...
      <div className="flex gap-2 justify-center">
        ...
        <button
          ...
          className="px-3 py-1.5 rounded-lg bg-volt text-white font-semibold text-xs disabled:opacity-30"
        >
