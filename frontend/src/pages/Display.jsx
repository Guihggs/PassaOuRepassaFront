      ...
      <div className="flex-1 flex flex-col items-center justify-center text-center gap-8">
        ...
        {(state.status === "live" || state.status === "answering" || state.status === "revealed") && q && (
          <div className="w-full max-w-4xl space-y-8">
            ...
            {state.status === "live" && pressOrderWithTeams.length > 0 && (
              <p className="text-white/50 text-sm animate-pulse -mb-4">
                Aguardando as outras equipes apertarem… ({pressOrderWithTeams.length}/3)
              </p>
            )}
                ...
                {pressOrderWithTeams.map((p) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      {result ? (
                        <span className={`text-xs font-semibold ${result.correct ? "text-green-400" : "text-red-400"}`}>
                          {result.correct ? `✓ +${result.pointValue} pt` : "✗ errou"}
                        </span>
                      ) : (
